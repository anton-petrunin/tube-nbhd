...
Choose $r>0$.
Suppose $V\subset M$ is a maximal set of points (with respect to inclusion) on distance at least $r$ from each other.
...
As a result, we get a uniformly smooth isometric immersion $(M,g)\to \RR^d$, where $d= \tfrac12\cdot N_2\cdot n\cdot(3\cdot n+11)$.
...
The conditions are closely related to the so-called \emph{regular maps};
...
The argument presented by David Hume and Alessandro Sisto \cite[1.1]{hume-sisto} can be used to find some necessary conditions; for example, \textit{the product of the hyperbolic plane and the real line does not admit a tubed embedding into hyperbolic space of arbitrary dimension}.
